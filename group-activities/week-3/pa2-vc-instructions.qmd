...
5. The Developer should then create a Version Control R Project on their computer and then download the `pa2-base-r-vc.qmd` (on Canvas) file into the folder on your computer.  
6. Test you connection by staging, committing, and pushing the new file (.Rproj and .qmd) to the GitHub Repository.  
...
## Part 5: Preview the `tidyverse`  
If you want to see what the above code would look like done completely in the `tidyverse` you can check out the [PA 2: Modernizing the Process](pa2-vc-intro-tidy-solution.qmd). You will notice that in some cases the `tidyverse` is much easier to understand and is more efficient (less code) while in other cases `base` R is more efficient. We are always trying to find the best tool and somethings that is a `tidyverse` tool and other times it will be a `base` tool.  
